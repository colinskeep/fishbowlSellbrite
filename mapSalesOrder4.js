  ...
  const [rows,fields] = await db.execute(`SELECT * FROM fba`);
    for(i = 0; i < data.length; i++){
      try{
        ...
        const [rows,fields] = await db.execute(`SELECT * FROM fba`);
        const location = rows.indexOf(data[i].items[0].order_item_ref) > -1 ? 'FBA' : 'MAIN';
        ...
        if(data[i].display_ref.startsWith('S') == false && data[i].display_ref != '97157-2790026192970' && data[i].display_ref != '91618-3793716125391' && data[i].channel_name != 'Walmart'){
          if(socount == 0){
            arr =  [];
