  ...
  const [rows,fields] = await db.execute(`SELECT * FROM fba`);
    for(i = 0; i < data.length; i++){
      try{
        ...
        if(data[i].display_ref.startsWith('S') == false && data[i].display_ref != '97157-2790026192970' && data[i].channel_name != 'Walmart'){
          if(socount == 0){
            arr =  [];
